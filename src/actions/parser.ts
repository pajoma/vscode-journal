...
export class Parser {
  ...
  private resolveShortcutString(value: string): number {
    if (value.match(/today|tod|heute|0/)) {
      return 0;
    }
    if (value.match(/tomorrow|tom|morgen/)) {
      return +1;
    }
    if (value.match(/yesterday|yes|gestern/)) {
      return -1;
    }
    return NaN;
  }
  ...
  public resolveISOString(value: string): number {
    ...
    if (!isNullOrUndefined(month!) && (month! < 0 || month! > 12)) {
      throw new Error("Invalid value for month");
    }
    if (!isNullOrUndefined(day!) && (day < 0 || day > 31)) {
    ...
    let result: number = Math.floor(
      (inputInMS - todayInMS) / (1000 * 60 * 60 * 24)
    );
    return result;
  }
  ...
  private getExpression() {
    ...
    if (isNullOrUndefined(this.expr)) {
      ...
      let weekdayRX =
        "(?:(next|last|n|l)?\\s?(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|wed|thu|fri|sat|sun|montag|dienstag|mittwoch|donnerstag|freitag|samstag|sonntag)\\s?)";

      let completeExpression: string =
        "^" +
        flagsRX +
        "?(?:" +
        shortcutRX +
