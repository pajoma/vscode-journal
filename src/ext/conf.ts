...
export class Configuration {
  ...
  public getLocale(): string {
    ...
    return isNullOrUndefined(locale) || locale!.length === 0
      ? "en-US"
      : locale!;
  }
  ...
  public getScopes(): string[] {
    let scopes: ScopeDefinition[] | undefined = this.config.get<
      [ScopeDefinition]
    >("scopes");
   ...
   * @param _scopeId
   */
  public getBasePath(scope: string): string {
    if (scope == SCOPE_DEFAULT) {
      ...
      if (!isUndefined(scopes)) {
        ...
        if (base.length == 0) return this.getBasePath(SCOPE_DEFAULT);
   ...
   * @param _scopeId
   */
  public getFileExtension(scope: string): string {
    if (scope != SCOPE_DEFAULT) {
      console.error(
        "You requested a scoped file extensions, this is not supported."
      );
    }
   ...
   */
  public getNotesPathPattern(
    date: Date,
    scope: string
  ): Q.Promise<ScopedTemplate> {
    return Q.Promise((onSuccess, onError) => {
      try {
        ...
        if (scope == SCOPE_DEFAULT) {
          definition = this.config.get<PatternDefinition>("pattern")?.notes
            ?.path;
        } else {
          definition = this.config
            .get<ScopeDefinition[]>("scopes")
            ?.filter((sd) => sd.name == scope)
            .pop()?.patterns?.notes?.path;
          scopedTemplate.scope = scope!;
        }
        ...
        scopedTemplate.value = this.replaceVariableValue(
          "homeDir",
          os.homedir(),
          scopedTemplate.value
        );
        scopedTemplate.value = this.replaceVariableValue(
          "base",
          this.getBasePath(scope),
          scopedTemplate.value
        );
        scopedTemplate.value = this.replaceDateFormats(
          scopedTemplate.value,
          date
        );
  ...
  public getEntryPathPattern(
    ...
    return Q.Promise((onSuccess, onError) => {
      try {
        ...
        if (scope == SCOPE_DEFAULT) {
          definition = this.config.get<PatternDefinition>("patterns")?.entries
            ?.path;
        } else {
          definition = this.config
            .get<ScopeDefinition[]>("scopes")
            ?.filter((sd) => sd.name == scope)
            .pop()?.patterns?.entries?.path;
          scopedTemplate.scope = scope!;
        }
  ...
  public getEntryFilePattern(
    ...
    return Q.Promise((onSuccess, onError) => {
      try {
        ...
        if (scope == SCOPE_DEFAULT) {
          definition = this.config.get<PatternDefinition>("pattern")?.entries
            ?.file;
        } else {
          definition = this.config
            .get<ScopeDefinition[]>("scopes")
            ?.filter((sd) => sd.name == scope)
            .pop()?.patterns?.entries?.file;
          scopedTemplate.scope = scope!;
        }
  ...
  // fix for #52
  // private regExpDateFormats: RegExp = new RegExp(/\$\{(?:(year|month|day|localTime|localDate|weekday)|(d:\w+))\}/g);
  private regExpDateFormats: RegExp = new RegExp(
    /\$\{(?:(year|month|day|localTime|localDate|weekday)|(d:[\s\S]+?))\}/g
  );

  private replaceDateFormats(template: string, date: Date): string {
    let matches: RegExpMatchArray =
      template.match(this.regExpDateFormats) || [];
  ...
  public getInputLabelTranslation(code: number) {
    ...
    let val = this.labelTranslations.get(
      this.getLocale().substring(0, 2) + code
    );
  ...
  public getInputDetailsTranslation(code: number): string | undefined {
    if (this.descTranslations.size == 0) {
      ...
      this.descTranslations.set(
        "en" + 3,
        "Select from the last journal entries."
      );
      this.descTranslations.set(
        "en" + 4,
        "Create a new note or select from recently created or updated notes."
      );
      this.descTranslations.set(
        "en" + 5,
        "Select from the list of recently added attachements."
      );
      ...
      this.descTranslations.set(
        "de" + 3,
        "Wählen Sie aus den letzten Journaleinträgen aus. "
      );
      this.descTranslations.set(
        "de" + 4,
        "Erstellen Sie eine neue Notiz oder wählen Sie aus den letzten Notizen aus."
      );
      this.descTranslations.set(
        "de" + 5,
        "Wählen Sie aus der Liste der zuletzt hinzugefügten Anlagen aus."
      );
      ...
      this.descTranslations.set(
        "fr" + 3,
        "Sélectionnez l'une des dernières entrées."
      );
      this.descTranslations.set(
        "fr" + 4,
        "Créez une nouvelle note ou sélectionnez une note parmi les notes récemment créées ou mises à jour."
      );
      this.descTranslations.set(
        "fr" + 5,
        "Sélectionnez dans la liste des pièces jointes récemment ajoutées"
      );
      ...
      this.descTranslations.set(
        "es" + 3,
        "Seleccione una de las últimas entradas. "
      );
      this.descTranslations.set(
        "es" + 4,
        "Cree una nueva nota o seleccione una de las notas creadas o actualizadas recientemente."
      );
      this.descTranslations.set(
        "es" + 5,
        "Seleccione de la lista de archivos adjuntos añadidos recientemente"
      );
    }
    let val = this.descTranslations.get(
      this.getLocale().substring(0, 2) + code
    );
    if (isUndefined(val)) val = this.labelTranslations.get("en" + code);
    return <string>val;
   ...
   */
  public getEntryTemplate(
    date: Date,
    scope: string
  ): Q.Promise<HeaderTemplate> {
    return this.getInlineTemplate(
      ...
      sp.value = this.replaceVariableValue(
        "base",
        this.getBasePath(scope),
        sp.value
      );
   ...
   * @memberof Configuration
   */
  public getNotesTemplate(scope: string): Q.Promise<HeaderTemplate> {
    return this.getInlineTemplate(
      "note",
      "# ${input}\n${tags}\n",
      scope
   ...
   * @memberof Configuration
   */
  public getFileLinkInlineTemplate(
    scope: string
  ): Q.Promise<InlineTemplate> {
    return this.getInlineTemplate(
      "files",
      "- Link: [${title}](${link})",
      scope
   ...
   * @memberof Configuration
   */
  public getMemoInlineTemplate(scope: string): Q.Promise<InlineTemplate> {
    return this.getInlineTemplate(
      "memo",
      "- MEMO: ${input}",
      scope
   ...
   * @memberof Configuration
   */
  public getTaskInlineTemplate(scope: string): Q.Promise<InlineTemplate> {
    return this.getInlineTemplate(
      "task",
      "- [ ] ${input}",
      scope
   ...
   * @memberof Configuration
   */
  public getTimeStringTemplate(scope: string): Q.Promise<ScopedTemplate> {
    return this.getInlineTemplate(
      "time",
      "LT",
      scope
  ...
  public isOpenInNewEditorGroup(): boolean {
    let res: boolean | undefined = this.config.get<boolean>(
      "openInNewEditorGroup"
    );
      ...
      try {
        ...
        if (Util.stringIsNotEmpty(this.config.get<string>("tpl-" + _id))) {
          resolve({
            ...
            after: Util.stringIsNotEmpty(
              this.config.get<string>(_id + "-after")
            )
              ? this.config.get<string>(_id + "-after")!
              : "",
          });
        ...
        if (scope == SCOPE_DEFAULT) {
          pattern = this.config
            .get<InlineTemplate[]>("templates")
            ?.filter((tpl) => tpl.name == _id)
            .pop();
        } else {
          ...
          this.config
            ...
            ?.filter((sd) => (sd.name = scope))
            .pop()
            ?.templates?.filter((tpl) => tpl.name == _id)
            ?.pop();
        }
        ...
        if (Util.isNullOrUndefined(pattern)) {
          ...
          if (Util.isNullOrUndefined(pattern?.template))
  ...
  private loadPatternsX(): Q.Promise<boolean> {
    return Q.Promise<boolean>((resolve, reject) => {
      ...
      let config: PatternDefinition | undefined = this.config.get<
        PatternDefinition
      >("patterns");
